 ...
 * @LastEditors: F1
 * @LastEditTime: 2021-09-18 22:56:34
 * @Description:
 *
 *				imbaseprotocls　主要应用场景是边缘端与云端通讯时，采用socket来同步数据，该项目主要为底层协议及通讯实现。应最大限度的避开业务逻辑。
 *				核心为三大部分:
		...
		for {
			...
			c := &ClientSocket{
				...
				OnRecvMessage: func(h protocols.Header, d []byte, cs *ClientSocket) {
					if h.Cmd != protocols.HEARTBEAT_REQUEST {
				...
				OnSendToMessage: func(sn string, sendTo *protoc.SendTo, cs *ClientSocket) {
					...
					go func() {
						...
						case reply.Data = <-syncMessage[sendTo.CID]:
							...
							replyData, _ := cs.InitMessage(protocols.SENDTO_RESPONSE, protocols.HEADER_FLAG_DATA_TYPE_PB, d)
