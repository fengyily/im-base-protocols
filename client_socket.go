 ...
 * @LastEditors: F1
 * @LastEditTime: 2021-09-18 22:56:12
 * @Description:
 *
...
func (cs *ClientSocket) HeartBeat() {
	go func() {
		for {
			...
			cs.SendMessage(protocols.HEARTBEAT_REQUEST, 0, nil)
			time.Sleep(5 * time.Second)
		}
	}()
}
	...
	for {
		...
		if cs.conn != nil {
			...
			for i := 0; i < len(cs.Buffer); {
				...
				if header.Cmd == protocols.HEARTBEAT_REQUEST {
					// 收到心跳包，马上回复一个
					go cs.SendMessage(protocols.HEARTBEAT_RESPONSE, 0, nil)
					//cs.Buffer = cs.Buffer[i+1:]
					if cs.OnRecvMessage != nil {
						cs.OnRecvMessage(header, nil, cs)
					}
					continue
				} else if header.Cmd == protocols.HEARTBEAT_RESPONSE {
					//fmt.Println("收到心跳回复。")
					continue
				} else {
					total := len(cs.Buffer)
					...
					if balance >= int(header.Length) {
						...
						if cs.OnRecvMessage != nil {
							...
							if header.Cmd == protocols.SENDTO_REQUEST {
...
func (cs *ClientSocket) SendToMessage(sn string, seq string, timeout time.Duration, body []byte) (err error) {
	...
	sendTo, _ := cs.InitMessage(protocols.SENDTO_REQUEST, protocols.HEADER_FLAG_IS_COMPRESS|protocols.HEADER_FLAG_DATA_TYPE_PB, d)
